...
## September 15, 2026: displayed illustrative RSI productivities

The default manuscript now displays newly solved chi=7.5 and chi=1.5
comparisons, each with sigma=0.9, 1, 1.1 and 1.5. These are illustrative
choices, not empirical estimates or moment fits. Omega_X=0.10, eta=0.20,
Bbar=1360.9921392415592, B0=0.01 Bbar and the four pre-RSI BGP stocks
are unchanged. Only chi differs between the two exercises.

All eight initial/refined solves and second horizon extensions were executed.
The existing algorithm and admission tolerances were retained. Both sigma=1.5
paths pass the two Hamiltonian-support grids and analytical continuation bound;
the other six pass the concavity test. Every path passes the independent
full-horizon and 801-date first-decade equation checks, activation continuity,
and asymptotic transversality checks. Largest dynamic residual: 1.689e-9;
largest last-horizon coordinate change: 4.295e-8. Final horizons range from
3013.0 to 6047.0 years, well beyond the displayed 500-year window.

At sigma=1.5, labor's income share reaches half its initial value at years
47.472886 and 212.711417, respectively. At unit elasticity, first-year research
expenditure is 0.18159784% and 0.06593060% of output. These moments are outputs,
not targets. The lower-chi case remains noticeably above its asymptotic growth
and interest rates at year 500; the paper explicitly distinguishes that
endpoint from the analytical limit.

Entry point: `scripts/simulate_rewrite_illustrative_rsi.py`; paper numbers and
the combined accuracy table are generated by `scripts/report_rewrite_illustrative_rsi.py`.
The two new folders `numerical_rewrite/rsi_chi_7_5/` and `rsi_chi_1_5/`
contain the full audit trail and data; figures use corresponding new prefixes.
Earlier data and figures were not overwritten. The editorial switch
`showcalibratedrsiscenarios` restores the former two displayed exercises.

Validation: 118 regression tests run, 117 passed and one legacy diagnostic
skipped. The five new design/admission/provenance tests all passed. Tectonic
0.16.9 compiled `main_rewrite.tex` into 67 pages without unresolved references
or overfull/underfull boxes. Pages 25--32 and 60 were rendered and visually
inspected: the parameter table, all six figures, and numerical-accuracy table
are legible and unclipped. Title, abstract, introduction, model and analytical
results were not changed. Section 6.2 starts on page 25 (figures 26--28),
Section 6.3 on page 29 (figures 30--32).

The remainder of this record documents preserved earlier experiments; its
publication descriptions refer to their respective historical versions.
