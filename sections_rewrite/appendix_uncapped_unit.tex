...
Section~\ref{subsec:rewrite-uncapped-unit-bgp} and gives the local conditions
for its convergence result. I use the uncapped research law
...
The local convergence part of Proposition~\ref{prop:rewrite-uncapped-unit-bgp}
uses two conditions. First, the linearized dynamics have exactly two stable
...
\end{equation}
Second, these stable directions must allow independent changes in the two
predetermined stocks. Let $V_S\in\mathbb R^{4\times2}$ be any real basis
matrix for $E^s$, and let $V_{KB}$ consist of its first two rows. The condition is
\begin{equation}
 \det V_{KB}\ne0.
...
rather than only for stocks on a curve. The two conditions are sufficient
local restrictions; the proof does not infer them from the BGP growth
identities alone. The complete argument, including both transversality
