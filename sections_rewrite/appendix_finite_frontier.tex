\paragraph{Preliminary results for Propositions~\ref{prop:rewrite-capped-complements-existence}--\ref{prop:rewrite-capped-substitutes-existence}.}
...
\]
The equation $y/k=(\rho+\gamma+\delta)/\alpha$ gives
...
\paragraph{Common positive-labor-share construction.}
...
\paragraph{AI-dominated construction.}
...
\paragraph{Optimality and transversality.}
...
household budget is linear at given prices and distributions, and the Euler and transversality conditions
...
domain at every date. Together with Assumption~\ref{ass:rewrite-research-concavity} and the verified
...
stocks. At $\overline B=\mathcal B(\sigma)$ the interior-share terminal
...
\begin{proof}[Proof of Proposition~\ref{prop:rewrite-capped-complements-existence}]
For $0<\sigma<1$ and $\overline B>\mathcal B(\sigma)$, the share map above
has a unique interior root. Equations~\eqref{eq:rewrite-labor-terminal-values}
and~\eqref{eq:rewrite-labor-terminal-research} construct the positive terminal
values. The common positive-labor-share construction gives the local stable
manifold and the unique nearby choices of $C_0$ and $q_0$. The optimality and
transversality argument then verifies that these paths satisfy
Definition~\ref{def:rewrite-equilibrium}, with the limits stated in
\eqref{eq:rewrite-complements-terminal}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:rewrite-capped-unit-existence}]
At $\sigma=1$, the exact static solution above determines positive terminal
values for every finite $\overline B>0$. The common positive-labor-share
construction again gives the local stable manifold and the unique nearby
choices of $C_0$ and $q_0$. The optimality and transversality argument verifies
Definition~\ref{def:rewrite-equilibrium} and yields
\eqref{eq:rewrite-unit-terminal}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:rewrite-capped-substitutes-existence}]
For $\sigma>1$, the share map above is strictly decreasing. If
$\overline B<\mathcal B(\sigma)$, it has a unique interior root and the common
positive-labor-share construction applies. If
$\overline B>\mathcal B(\sigma)$, the AI-dominated construction gives the
local stable manifold, the unique nearby choices of $C_0$ and $q_0$, and the
limits in \eqref{eq:rewrite-substitutes-ai-limits} and
\eqref{eq:rewrite-substitutes-wage-growth}. The common optimality and
transversality argument verifies Definition~\ref{def:rewrite-equilibrium} in
both cases. At $\overline B=\mathcal B(\sigma)$, the boundary fixed point is
not hyperbolic, so the argument does not apply.
\end{proof}
