\paragraph{Proof strategy for Propositions~\ref{prop:rewrite-capped-unit-existence}--\ref{prop:rewrite-capped-substitutes-existence}.}
\label{app:rewrite-finite-existence}

The proofs have three steps. First, Lemma~\ref{lem:rewrite-monopoly-choice}
...
The frontier makes this possible for initial efficiency sufficiently
...
\paragraph{Candidate long-run values.}
...
Use the state coordinates $k,d,\tau$ introduced before
Proposition~\ref{prop:rewrite-capped-unit-existence}, together
with consumption and the AI-efficiency shadow price divided by $AN$:
\begin{equation}
...
initial-state neighborhood $\mathcal U_L$. Every physical initial state
satisfying \eqref{eq:rewrite-labor-local-initial} has a solution on this
...
Consequently $g_Y,g_X,g_C,g_K\to\overline g$. Also $s_X\to1$ and
...
\]
Shrink $\mathcal U_L$ or $\mathcal U_A$ so that $B_0\geq b_{**}$.
...
\begin{proof}[Proof of Proposition~\ref{prop:rewrite-capped-substitutes-labor-existence}]
For $\sigma>1$, the share map above is strictly decreasing. If
$\overline B<\mathcal B(\sigma)$, it has a unique interior root and the
common positive-labor-share construction gives convergent trajectories.
The common optimality and transversality argument verifies
Definition~\ref{def:rewrite-equilibrium} and yields the limits in
\eqref{eq:rewrite-complements-terminal}.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:rewrite-capped-substitutes-existence}]
For $\sigma>1$ and $\overline B>\mathcal B(\sigma)$, the AI-dominated
construction gives the local stable manifold, the unique nearby choices
of $C_0$ and $q_0$, and the limits in
\eqref{eq:rewrite-substitutes-ai-limits} and
\eqref{eq:rewrite-substitutes-wage-growth}. The common optimality and
transversality argument verifies Definition~\ref{def:rewrite-equilibrium}.
At $\overline B=\mathcal B(\sigma)$, the boundary fixed point is not
hyperbolic, so the argument does not apply.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:rewrite-output-wage-wedge}]
Propositions~\ref{prop:rewrite-capped-unit-existence}--\ref{prop:rewrite-capped-substitutes-labor-existence}
imply $g_w\to\gamma$ whenever the limiting labor share is positive.
Proposition~\ref{prop:rewrite-capped-substitutes-existence} implies both $g_w\to\gamma+(\overline r-\rho-\gamma)/\sigma>\gamma$ and
