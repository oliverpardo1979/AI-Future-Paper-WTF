...
\]
Therefore labor's income share therefore converges to zero, and the AI industry's share on revenue share converges to $1-\alpha$.
%\[
...
Table~\ref{tab:rewrite-finite-existence} summarizes the four cases of
Proposition~\ref{prop:rewrite-equilibrium-regimes}.
...
\begin{tabular}{@{}ccccc@{}}
\toprule
Elasticity & AI efficiency bounds & $\lim g_{Y/N}$ & $\lim wL/Y$ & $\lim r$ \\
\midrule
$0<\sigma<1$ & $\overline B>\mathcal B(\sigma)$ & $\gamma$ & Positive & $\rho+\gamma$ \\
$\sigma=1$ & Any finite $\overline B$ & $\gamma$ & $(1-\alpha)\omega_L$ & $\rho+\gamma$ \\
$\sigma>1$ & $\overline B<\mathcal B(\sigma)$ & $\gamma$ & Positive & $\rho+\gamma$ \\
$\sigma>1$ & $\overline B>\mathcal B(\sigma)$ & $\mathcal R(\overline B) -\rho>\gamma$ & Zero & $\mathcal R(\overline B)>\rho+\gamma$ \\
\bottomrule
\end{tabular}
\end{table}

For $\sigma>1$, a frontier below $\mathcal B(\sigma)$ makes the return
at a vanishing labor share too low for capital and AI services to outgrow
effective labor. Above the threshold, that return supports their faster
growth along the AI-dominated equilibria.
...
developer's value is bounded on every finite horizon. A trajectory that
ends at a finite date cannot satisfy Definition~\ref{def:rewrite-equilibrium},
which requires finite, feasible allocations for every $t\geq0$.
