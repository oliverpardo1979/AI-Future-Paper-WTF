...
\subsection{The return to capital and the growth threshold}
\label{subsec:rewrite-return-threshold}

A finite frontier bounds AI efficiency, but not the compute used to supply
AI production services. To identify the return when labor's income share
vanishes, fix $B>0$ and consider $s_X\to1$, with $\sigma\ne1$.
Since $wL/Y=(1-\alpha)(1-s_X)$, this is the zero-labor-share limit.

Combining the final-good firm's demand for AI services,
...
\]
The limit uses $e_X\to\alpha$ from
\eqref{eq:rewrite-inverse-demand-elasticity}. One factor $1-\alpha$ comes
from the marginal product of AI services; the other comes from the
developer's marginal-revenue condition. The CES aggregator and its
elasticity, equations~\eqref{eq:rewrite-ces} and \eqref{eq:rewrite-sx}, give
\[
...
\]
Using $r=\alpha Y/K-\delta$, equation~\eqref{eq:rewrite-capital-return},
...
\end{equation}
This is a return at a particular limit of the production and pricing
conditions, not the interest rate at an arbitrary date. The actual
interest rate also depends on capital relative to effective labor.
...
$r\to\mathcal R(\overline B)$; the calculation alone does not establish
that such a trajectory exists.

The relevant comparison is with $\rho+\gamma$. By the Euler equation,
...
increasing, define $\mathcal B(\sigma)$ for $\sigma\ne1$ by
\begin{equation}
 \begin{aligned}
...
\end{equation}
The first line equates the zero-labor-share return to the return that
sustains growth at $\gamma$; the second solves for AI efficiency.
I interpret this threshold as a boundary between growth regimes only
for $\sigma>1$. With complementarity, it instead enters a sufficient
condition for the construction below. At $\sigma=1$,
\eqref{eq:rewrite-ai-terminal-ratio} is undefined and I use the
Cobb--Douglas limit of the CES aggregator.
...
The next proposition constructs equilibrium trajectories, not just
balanced-growth paths.

\Needspace{7\baselineskip}
\begin{proposition}[Equilibrium existence and long-run regimes]
\label{prop:rewrite-equilibrium-regimes}
Under the parameter restrictions of Section~\ref{sec:rewrite-model},
fix $0<\overline B<\infty$. For each parameter configuration covered by
a case below, there exists a nonempty open set of initial conditions
$(A_0,N_0,K_0,B_0)$, with $A_0,N_0,K_0>0$ and
$0<B_0<\overline B$, such that every initial state in that set has an
equilibrium trajectory in the sense of
Definition~\ref{def:rewrite-equilibrium}, defined for all $t\geq0$,
...
\begin{enumerate}[label=(\roman*),ref=\theproposition(\roman*),leftmargin=*,itemsep=0.5em]
 ...
 \begin{equation}
  ...
  r\to\rho+\gamma,\qquad s_X\to\overline s_X\in(0,1).
 ...
 If $\sigma=1$, then
 \begin{equation}
  g_Y-n\to\gamma,\qquad g_w\to\gamma,\qquad r\to\rho+\gamma,
  \label{eq:rewrite-unit-terminal}
 \end{equation}
 and $s_X=\omega_X$ at every date.
 ...
 If $\sigma>1$ and $\overline B>\mathcal B(\sigma)$, then
 \begin{gather}
  g_Y-n\to\mathcal R(\overline B)-\rho>\gamma,
 ...
 \end{gather}
\end{enumerate}
The initial-state sets and the constants $\overline s_X$ depend on the
parameters and may differ across cases. The proposition does not cover
$\sigma>1$ with $\overline B=\mathcal B(\sigma)$.
\end{proposition}
...
The identity $wL/Y=(1-\alpha)(1-s_X)$ translates these limits into income
shares. Labor retains a positive share in the first three cases and a
vanishing share in the fourth. At unit elasticity, its share is exactly
$(1-\alpha)\omega_L$ at every date; correspondingly,
$p_XX/Y=(1-\alpha)\omega_X$, whereas $s_X=\omega_X$.
Write $\overline r=\lim_{t\to\infty}r_t$ along a constructed equilibrium.
Table~\ref{tab:rewrite-finite-existence} summarizes the four cases.
...
Figure~\ref{fig:rewrite-frontier-regimes} illustrates the two constructed
growth regimes for $\sigma>1$ and the threshold separating them.
...
\subsection{Wages and labor's income share}
\label{subsec:rewrite-wages-shares}
...
Proposition~\ref{prop:rewrite-equilibrium-regimes}. Then
\[
...
\begin{proof}
See Appendix~\ref{app:rewrite-proofs},
\hyperref[proof:rewrite-output-wage-wedge]{Proof of
Proposition~\ref*{prop:rewrite-output-wage-wedge}}.
\end{proof}
