...
Capital and inference compute can expand together, sustaining growth without
permanently rising AI efficiency. RSI determines how efficiency improves
toward a level at which this mechanism supports growth faster than $\gamma$.

The omission of the case where $\sigma<1$ and $\overline B < \mathcal B(\sigma)$ is deliberate as the upper bound $\overline B$ on AI efficiency is a device to achieve an equilibrium when $\sigma>1$.
It can be shown that in the omitted case any equilibrium has an AI production bottleneck, meaning that long-run output-per-worker growth is lower than $\gamma$. %I do not show the proof of this statement since it is analogous to case \ref{prop:rewrite-capped-substitutes-existence} in Proposition \ref{prop:rewrite-equilibrium-regimes}.
%See Appendix~\ref{app:rewrite-proofs}, \hyperref[proof:rewrite-low-cap-complements]{Proof of the AI-bottleneck statement}.
...
Rising real wages alongside a declining labor share also appear in
\citet[Proposition~5]{acemoglurestrepo2018} and
...
real wages grow than labor-augmenting technological progress.

\citet{acemoglurestrepo2018} also show that the creation of new tasks in
which labor has a comparative advantage can counteract automation and
sustain a positive labor income share. I hold the production weights fixed
and do not model the creation of new human tasks. Whether labor's share
would still vanish with this adjustment remains an open question.
