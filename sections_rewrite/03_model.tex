...
\end{equation}
Admissible choices satisfy $C_t>0$, $K_t\geq0$, and $K(0)=K_0$.
...
Admissible research paths have $M_t\geq0$ and finite total expenditure on
every finite interval. The objective is the present value of net
distributions, not the flow $\Pi$ at a particular date.

I use two restrictions on the research technology, distinguishing below
the results that require them from those that do not.
...
variable.
Section~\ref{sec:rewrite-regimes} shows that neither restriction is needed
for local equilibrium existence sufficiently near a fixed finite frontier:
there the frontier itself supplies the curvature needed for global
...
not by themselves establish global optimality.
Lemma~\ref{lem:rewrite-developer-verification} in
Appendix~\ref{app:rewrite-proofs} gives a global verification result for the
developer's research path. It uses a monotone transformation of the bounded
AI-efficiency state and requires the maximized Hamiltonian to have a global
supporting tangent at the candidate state. Global concavity is sufficient for
this condition but is not necessary. The equilibrium existence proofs use a
concavity test, whereas the quantitative analysis checks the weaker support
condition directly whenever global concavity fails.
...
\end{subequations}
\end{enumerate}
The exogenous paths are $A_t=A_0e^{\gamma t}$ and $N_t=N_0e^{nt}$,
and the predetermined stocks satisfy $K(0)=K_0$ and $B(0)=B_0$.
At $\sigma=1$, the CES equations use their continuous limits
$Z=(AL)^{\omega_L}X^{\omega_X}$ and $s_X=\omega_X$.
The trajectory is feasible for the agents' stated problems, the household
and developer attain their optima, and final-good firms maximize profit.
For a finite frontier, $B_t<\overline B$ at every finite date.
