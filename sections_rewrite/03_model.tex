...
Competitive firms produce the final good from physical capital $K$, effective labor $AL$, and effective AI production services $X$. Let $0<\alpha<1$ denote the capital elasticity, $\delta\geq0$ the depreciation rate, and $\sigma>0$ the elasticity of substitution between effective labor and effective AI production services. The CES weights $\omega_L$ and $\omega_X$ are strictly positive. Production is
\begin{align}
 Y&=K^\alpha Z^{1-\alpha},
...
Problem~\eqref{eq:rewrite-service-choice} has a unique global maximizer
$X^*>0$.
\end{lemma}
...
$R'(X^*)=1/B$. The strict second-order condition
\eqref{eq:rewrite-monopoly-soc} therefore
implies
\begin{equation}
...
Appendix~\ref{app:rewrite-proofs} establishes these results.
...
Appendix~\ref{app:rewrite-proofs} uses this property after a
...
For a finite AI-efficiency frontier, Proposition~\ref{prop:rewrite-developer-sufficiency} in Appendix~\ref{app:rewrite-proofs} provides a sufficient test
