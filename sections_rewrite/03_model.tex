...
$(BM)^\eta$ jointly concave in $B$ and $M$. After deriving the developer's
necessary conditions, I use this property to give a sufficient condition for
global optimality with a finite frontier. The assumption is not, by itself, a
concavity condition for the full AI-efficiency law in the original state
variable.
...
The preceding conditions are necessary for an interior optimum, but they do
not by themselves establish global optimality. To state a sufficient
condition, fix a date $t$ and an AI-efficiency level
...
Let $s_{X,t}(b)$ and $e_{X,t}(b)$ be the corresponding values defined by
Equations~\eqref{eq:rewrite-sx} and
\eqref{eq:rewrite-inverse-demand-elasticity}. Define
\begin{equation}
...
\end{equation}
The endogenous elasticity $\varepsilon_{B,t}(b)$ measures the percentage
change in the developer's optimal sales of AI production services caused by a
one-percent increase in AI efficiency, holding $K_t,A_t,$ and $L_t$ fixed. It
is not an additional parameter of the model.

\Needspace{20\baselineskip}
\begin{proposition}[Sufficient condition for global developer optimality]
\label{prop:rewrite-developer-sufficiency}
Fix $0<B_0<\overline B<\infty$ and paths
$\{K_t,A_t,L_t,r_t\}_{t\geq0}$, with $K_t,A_t,L_t>0$. Let
$\{B_t,M_t,X_t,q_t\}_{t\geq0}$ be paths such that $B_t$ and $q_t$ are
continuously differentiable and $M_t,X_t,q_t>0$. Suppose the integral in
problem~\eqref{eq:rewrite-developer-reduced} is finite and, for every
$t\geq0$:
\begin{enumerate}[label=(\roman*)]
 \item $X_t=X_t^*(B_t)$ solves the static problem
 \eqref{eq:rewrite-service-choice} and therefore satisfies
 \eqref{eq:rewrite-monopoly-foc};
 \item $B(0)=B_0$, $B_t\in[B_0,\overline B)$, and $B_t$ satisfies the
 AI-efficiency law \eqref{eq:rewrite-capability-law};
 \item $M_t$ and $q_t$ satisfy the research condition
 \eqref{eq:rewrite-research-foc} and the costate equation
 \eqref{eq:rewrite-costate}; and
 \item the transversality condition \eqref{eq:rewrite-developer-tvc} holds.
\end{enumerate}
Under Assumption~\ref{ass:rewrite-research-concavity}, suppose in addition that
\begin{equation}
 \psi(b)[\varepsilon_{B,t}(b)-2]
 \leq\frac{b}{\overline B}
 \label{eq:rewrite-capped-concavity-gate}
\end{equation}
for every $t\geq0$ and every $b\in[B_0,\overline B)$. Then
$\{B_t,M_t\}_{t\geq0}$ solves the reduced developer problem
\eqref{eq:rewrite-developer-reduced}. Together with
$X_t=X_t^*(B_t)$, it is globally optimal for the developer.
\end{proposition}
...
necessary. It holds strictly near the frontier because $\psi(b)$ converges to
zero while $b/\overline B$ converges to one. It can fail at lower
AI-efficiency levels, which is why the numerical equilibrium audit checks it at
every efficiency level that the developer could reach from $B_0$.
