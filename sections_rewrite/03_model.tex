...
I use \emph{compute} to denote the computational resources used to execute the numerical operations required by AI systems. Operational measures include arithmetic operations and accelerator-hours \citep{sastryetal2024}. Because time is continuous, the variables $U$ and $M$ below are rates of compute use---operations or accelerator-hours per unit of time---rather than stocks of processors or data centers. I abstract from differences across hardware, energy use, and utilization rates, and measure compute in units such that one unit costs one unit of the final good. Compute is therefore a rival and costly input; it is not itself a measure of AI performance or intelligence.

I distinguish compute used to deploy AI from compute used to improve it. Inference compute $U$ runs existing AI systems and produces outputs used in current final-good production. Research compute $M$ is allocated to the development of better AI systems. It includes training and fine-tuning, but also computational experimentation, model evaluation, and other compute-intensive components of AI research. The term \emph{research compute} is therefore deliberately broader than \emph{training compute}. This distinction is close to the separation between inference and R\&D/training activities in \citet{korinekmckelvey2026}. The research use also captures the feedback studied by \citet{davidsonetal2026}, in which better AI accelerates further AI development.
...
Raw compute does not by itself determine the services produced. The variable $B$ measures AI efficiency: the quantity of effective AI services obtained from one unit of compute. Thus $U$ and $M$ are raw computational inputs, while $BU$ and $BM$ are the corresponding flows of AI production and AI research services. This distinction parallels measures of algorithmic efficiency based on the decline in compute required to achieve a given level of performance \citep{hernandezbrown2020,erdilbesiroglu2022}. It also separates expansion in physical compute from improvements in the algorithms that use it. I use the broader term \emph{AI efficiency} because the same $B$ augments compute in both activities. Inference compute produces AI production services according to
\begin{equation}
 X=BU.
...
For \(K,A,L,B>0\) and \(\omega_L,\omega_X>0\), condition
\eqref{eq:rewrite-monopoly-foc} selects the unique global maximizer \(X^*>0\).
Appendix~\ref{app:rewrite-monopoly-choice} gives the proof.
...
Let $R(X)=p_X(X)X$ and write the developer's unique within-date choice as
$X^*(B)$, leaving its dependence on $K,A,$ and $L$ implicit. Holding those
three variables fixed, condition~\eqref{eq:rewrite-monopoly-foc} gives
$R'(X^*)=1/B$. The strict second-order condition at the maximizer therefore
implies
\begin{equation}
 \frac{\partial X^*}{\partial B}
 =-\frac{1}{B^2R''(X^*)}>0.
 \label{eq:rewrite-ai-supply-efficiency}
\end{equation}
Define
$\varepsilon_{X,B}\equiv\partial\ln X^*/\partial\ln B>0$. Equations
\eqref{eq:rewrite-output}, \eqref{eq:rewrite-ces}, \eqref{eq:rewrite-sx}, and
\eqref{eq:rewrite-wage}, together with the chain rule, imply
\begin{align}
 \frac{\partial\ln w}{\partial\ln B}
 &=\left(\frac{1}{\sigma}-\alpha\right)s_X\varepsilon_{X,B},
 \label{eq:rewrite-wage-ai-efficiency-elasticity}\\
 \frac{\partial\ln (wL/Y)}{\partial\ln B}
 &=\left(\frac{1}{\sigma}-1\right)s_X\varepsilon_{X,B}.
 \label{eq:rewrite-labor-share-ai-efficiency-elasticity}
\end{align}
Hence greater AI efficiency raises the wage if $\sigma<1/\alpha$ and lowers it
if $\sigma>1/\alpha$, while it raises labor's income share if $\sigma<1$ and
lowers it if $\sigma>1$. When $1<\sigma<1/\alpha$, greater AI efficiency
therefore raises the wage while reducing labor's income share. This result
provides a concrete AI-production mechanism for the broader possibility
emphasized by \citet{autorkausik2026}: a falling labor income share can coexist
with a rising real wage. The thresholds $1$ and $1/\alpha$ are specific to the
production technology used here. These within-date results hold $K,A,$ and $L$
fixed.
\end{remark}
