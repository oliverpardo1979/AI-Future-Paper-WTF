...
not by themselves establish global optimality. For a finite frontier, define
research depth as the monotone transformation
\begin{equation}
...
Lemma~\ref{lem:rewrite-developer-verification} is a verification result, not
an existence theorem. It does not assume that the developer's entire problem
is globally concave. Condition~\eqref{eq:rewrite-hamiltonian-support} requires
only that the maximized Hamiltonian have a global supporting tangent at the
candidate state. Global concavity is one sufficient way to obtain this
support, but it is not necessary. Under
Assumption~\ref{ass:rewrite-research-concavity}, it is enough to verify that
$R_B\mapsto\pi_t(B(R_B))$ is concave. Appendix~\ref{app:rewrite-proofs}
derives an exact algebraic test for that stronger condition. The equilibrium
existence proofs use the concavity test, whereas the quantitative analysis
checks the weaker support condition directly whenever global concavity fails.
