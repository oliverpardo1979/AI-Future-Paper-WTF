...
Competitive firms produce the final good from physical capital $K$, effective labor $AL$, and effective AI production services $X$. Let $\delta\geq0$ be the depreciation rate and $\sigma>0$ the elasticity of substitution between effective labor and effective AI production services. The CES weights are denoted by
$\omega_L$ and $\omega_X$. Production is
\begin{align}
 Y&=K^\alpha Z^{1-\alpha},
...
\begin{remark}[On the role of the elasticity of substitution]
\label{rem:rewrite-ai-labor}
Equations~\eqref{eq:rewrite-output}, \eqref{eq:rewrite-ces},
\eqref{eq:rewrite-sx}, and \eqref{eq:rewrite-wage} imply that, holding $K$ and $AL$ fixed,
\begin{align}
 \frac{\partial\ln w}{\partial\ln X}
 &=\left(\frac{1}{\sigma}-\alpha\right)s_X,
 \label{eq:rewrite-wage-ai-elasticity}\\
 \frac{\partial\ln (wL/Y)}{\partial\ln X}
 &=\left(\frac{1}{\sigma}-1\right)s_X.
 \label{eq:rewrite-labor-share-ai-elasticity}
\end{align}
Because $s_X>0$, greater use of AI services raises the wage if
$\sigma<1/\alpha$ and lowers it if $\sigma>1/\alpha$, while it raises labor's
income share if $\sigma<1$ and lowers it if $\sigma>1$. Therefore, when
$1<\sigma<1/\alpha$, greater use of AI services raises the wage while reducing
labor's income share. This result matches a possibility emphasized by \citet{autorkausik2026}: a falling
labor income share can coexist with a rising real wage. %The thresholds $1$ and $1/\alpha$ are specific to the nested Cobb--Douglas--CES production technology used here.
%
%Holding the wage rather than labor fixed, equations~\eqref{eq:rewrite-output},
%\eqref{eq:rewrite-ces}, \eqref{eq:rewrite-sx}, and
%\eqref{eq:rewrite-wage} imply that conditional labor demand satisfies
%\begin{equation}
% \frac{\partial\ln L}{\partial\ln X}
% =
% \frac{(1/\sigma-\alpha)s_X}
% {\alpha(1-s_X)+s_X/\sigma}.
% \label{eq:rewrite-labor-demand-ai-elasticity}
%\end{equation}
%The denominator is positive, so conditional labor demand rises with $X$ if
%$\sigma<1/\alpha$ and falls if $\sigma>1/\alpha$. 
Of course, these effects are not even partial equilibrium results. However, they provide a
first illustration of why the elasticity of substitution matters for the
...
I maintain two restrictions on the research technology.
\begin{assumption}[Research scale]
...
\begin{assumption}[Research concavity]
\label{ass:rewrite-research-concavity}
\begin{equation}
 \eta\leq\frac{1}{2}.
 \label{eq:rewrite-eta-half}
\end{equation}
\end{assumption}

The two assumptions serve different purposes. Assumption~\ref{ass:rewrite-research-curvature}
limits the feedback from research expenditure to AI efficiency and then to
operating profit. Without a finite efficiency frontier, it ensures that, on a
fixed finite horizon and taking $K,A,L,$ and $r$ as given, a bound on the
operating profits generated by research grows less than proportionally with
total research expenditure, while its cost grows linearly. Arbitrarily large
expenditure therefore cannot be optimal. A finite frontier already bounds AI
efficiency, so this restriction is not needed for finite-horizon existence of
the developer's optimum in that case. I retain it because it also applies when
the frontier is removed. Proposition~\ref{prop:rewrite-research-scale} in
Appendix~\ref{app:rewrite-research-scale} establishes these results.

Assumption~\ref{ass:rewrite-research-concavity} makes the research term
$(BM)^\eta$ jointly concave in $B$ and $M$. For the finite-frontier technology,
Proposition~\ref{prop:rewrite-developer-sufficiency} in
Appendix~\ref{app:rewrite-developer-sufficiency} uses this property after a
change of state variable. Together with the operating-profit curvature
condition stated there, it makes the first-order and transversality conditions
sufficient for global optimality of a feasible developer policy with a finite
objective. The assumption is not, by itself, a concavity condition for the
full AI-efficiency law in the original state variable.
