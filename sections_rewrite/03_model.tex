...
Time is continuous.  The economy contains a representative household, competitive final-good firms, and an integrated AI developer. Labor-augmenting technology $A$ grows exogenously. AI productivity $B$ improves through autonomous AI research: the only current input used to raise $B$ is research compute. The model therefore describes an economy in which there are no longer human researchers developing AI and AI has achieved recursive self-improvement (RSI).
...
The representative household owns physical capital and the AI developer. Let \(\Pi\) denote the AI developer's net profit, which accrues to the household. The household takes the paths of the net interest rate \(r\), the wage \(w\) and \(\Pi\) as given. For a predetermined capital stock $K_0>0$, it chooses aggregate consumption $C$ to solve
\begin{equation}
 \max_{\{C_t\}_{t\geq0}}
...
Raw compute does not by itself determine the quantity of AI services produced. The variable $B$ measures AI productivity: AI services obtained per unit of compute. Thus $U$ and $M$ are raw computational inputs, while $BU$ and $BM$ are the corresponding flows of AI production and AI research services. This services-per-compute interpretation can also be described as \emph{AI efficiency}. It parallels the computer-science literature on \emph{algorithmic efficiency}, which measures reductions in the compute required to achieve a given level of performance \citep{hernandezbrown2020,erdilbesiroglu2022}. A rise in $B$ therefore represents an improvement in the algorithms that use compute, rather than an expansion of physical computing capacity. I assume that the same $B$ augments compute in both activities.
...
\begin{figure}[H]
 ...
 \begin{tikzpicture}[>=Latex,font=\small]
  ...
  \node[rewrite-state] (B) at (-7.5,-0.8) {\(B\)\\AI productivity};
 ...
 \caption{Production of final output and AI production services. AI productivity $B$ augments inference compute $U$ to produce $X=BU$, the flow of AI production services. These services combine with effective labor $AL$ to produce $Z$, which combines with capital $K$ to produce $Y$.}
...
Given initial AI productivity \(B_0>0\) and an upper bound
\(\overline B\in(B_0,\infty]\), AI productivity evolves according to the RSI technology
\begin{equation}
...
\begin{figure}[H]
 ...
 \begin{tikzpicture}[>=Latex,font=\small]
  ...
  \node[rewrite-state] (B) at (-5.9,-0.9) {\(B\)\\AI productivity};
  ...
  \node[rewrite-technology] (Bd) at (2.2,0.0)
   {\(\dot B=\chi(BM)^\eta\psi(B)\)\\productivity improvement};
  ...
  \draw[rewrite-dynamics] (Bd.east) -- (5.0,0.0) -- (5.0,-2.0) --
    (-5.9,-2.0) -- (B.south);
 \end{tikzpicture}%
 }
 \caption{Recursive self-improvement in AI productivity. AI productivity $B$ augments
...
The upper bound on AI productivity \(\overline B\) allows me to establish and compute equilibria for finite values of \(\overline B\) and then ask whether they have a meaningful limit as \(\overline B\to\infty\). Without the upper bound, recursive improvement may instead produce a finite-time singularity: one or more endogenous quantities---such as AI productivity, output, capital, or research compute---become unbounded at a finite date, so the trajectory cannot be continued as a path of finite allocations. %Section~\ref{subsec:rewrite-uncapped} explains why the uncapped economy requires a separate equilibrium analysis. %AI productivity may approach the upper bound asymptotically but cannot cross it in finite time. %Appendix~\ref{app:rewrite-cap-invariance} gives the derivation.
...
\end{equation}
%The $B^2$ term does not represent quadratic returns to AI productivity. It results from differentiating the inference cost $X/B$:
At a fixed $X$, a marginal increase in AI productivity saves $U/B$ units of the consumption good.

AI productivity is nonrival in use. Once $B$ has been developed, using it to
...
service providers had free access to $B$. Free entry would imply no operating rent. A developer that paid for $M$ would improve a common AI-productivity index $B$ that competing providers could use without compensating it. The developer would bear the research cost but could not appropriate the resulting benefits. %The problem is therefor one of research incentives, not the physical availability of compute.
...
The AI developer uses $M$ for research and owns $B$ and any resulting improvements in AI productivity. %Taking the paths of $r,K,A,$ and $L$ as given, it solves
%\begin{equation}
...
% \label{eq:rewrite-profit-envelope}
%\end{equation}
%The $B^2$ term does not represent quadratic returns to AI productivity. It results from differentiating the inference cost $X/B$: at a fixed $X$, a marginal increase in AI productivity saves $X/B^2$ units of compute.
...
the path of research compute $M$ to increase AI productivity $B$:
\begin{equation}
\max_{\{M_t\}_{t\geq0}}
 \int_0^\infty
 \exp\left\{-\int_0^t r_s\dd s\right\}
...
every finite interval. The objective is the present value of net profit,
not profit at a single date.
...
Let $q$ be the current-value shadow price of AI productivity, measured in units of
...
%\end{equation}
Equation~\eqref{eq:rewrite-research-foc} equates the marginal benefit of research compute to its unit cost. Holding current $B$ fixed, an additional unit of $M$ raises the rate of improvement in AI productivity by \(\chi\eta B(BM)^{\eta-1}\psi(B)\).
%The factor $B$ appears because raw compute $M$ produces $BM$ effective AI research services.
Multiplication by $q$ values the resulting productivity gain, accounting for future profit gains and further improvements in AI productivity. %The right-hand side is one because one unit of compute costs one unit of the final good. At an interior optimum, an additional unit of research therefore creates exactly enough value to cover its cost.
...
\end{equation}
The first term on the right-hand side of Equation~\eqref{eq:rewrite-capability-return} is the inference-compute saving from greater AI productivity. The
second is the research-productivity gain from RSI. With a finite upper bound, the third
term is negative because a higher $B$ leaves less room for further improvement. The final term is the capital gain on AI productivity. Together these
...
\end{equation}
It rules out a path that leaves positive discounted value in the AI-productivity state.%It does not require $q$, $B$, or their undiscounted product to converge to zero.
