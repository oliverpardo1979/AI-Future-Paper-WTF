...
I use \emph{compute} to denote the computational resources used to execute the numerical operations required by AI systems \citep{sastryetal2024}. The variables $U$ and $M$ are rates of compute use rather than stocks of processors or data centers. I measure compute in units such that its price in final goods is one. Compute is therefore a rival, costly input, not a measure of AI performance.
...
Under the maintained restrictions $0<\alpha<1$, $\sigma>0$, and
$\omega_L,\omega_X>0$, problem~\eqref{eq:rewrite-service-choice} has a
unique global maximizer $X^*>0$ for every $K,A,L,B>0$. This maximizer is the
unique solution to condition~\eqref{eq:rewrite-monopoly-foc}. If
$R(X)\equiv p_X(X)X$, it also satisfies $R''(X^*)<0$.
\end{lemma}

\begin{proof}
See Appendix~\ref{app:rewrite-monopoly-choice}.
\end{proof}
...
$R'(X^*)=1/B$. The strict second-order condition in
Lemma~\ref{lem:rewrite-monopoly-choice} therefore
implies
\begin{equation}
