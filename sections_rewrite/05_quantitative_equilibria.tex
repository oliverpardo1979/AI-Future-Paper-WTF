...
\begin{tabular}{P{0.08\textwidth}P{0.19\textwidth}P{0.27\textwidth}P{0.35\textwidth}}
\toprule
...
$\omega_X$ & 0.20 & Weight on AI services & Scenario choice, not an observed industry share\\
...
$\overline B$ & $1.10\,\overline B_c(1.50)$ $\simeq170.124$ & Common capability frontier & Ten percent above the threshold for $\sigma=1.50$\\
\bottomrule
\end{tabular}
\end{table}
...
state of the capped economy. An exact constant-capability reference at
$B_0=\overline B$ would eliminate further capability improvements and would
...
capabilities, but the Hamiltonian support test in
...
Figure~\ref{fig:rewrite-growth-distribution} presents the six main outcomes.
Dividing output by $AL$ removes population growth and exogenous productivity
growth; it does not force the AI-dominated trajectory to become stationary.
Similarly, a constant $w/A$ means that real wages grow at $\gamma$, not that
they stop growing. AI-industry revenue $p_XX/Y$ is distinct from profit and
from value added. Gross capital income remains $\alpha Y$ throughout, so the
labor and AI-revenue shares sum to $1-\alpha$.

\begin{figure}[H]
 \centering
 \includegraphics[width=\textwidth]{figures_rewrite/equilibrium_growth_distribution.pdf}
 \caption{Growth, wages, and the distribution of final output. Each panel
 compares the same four admitted numerical equilibrium trajectories over
 4,000 model years. Output per unit of effective labor and the wage relative
 to productivity use logarithmic vertical scales. Growth, net interest, and
 output shares use linear percentage scales. All scenarios have the same
 initial stocks, frontier, and parameters except $\sigma$.}
 \label{fig:rewrite-growth-distribution}
\end{figure}
...
at $2.423\%$, although the labor income share vanishes and AI revenue
approaches $67\%$ of output. These limiting rates describe the equilibrium
continuations; they are not imposed as constant rates during the transition.
...
Figure~\ref{fig:rewrite-technology-revenue} reports capability relative to its
frontier, consumption and capital per unit of effective labor, and the use of
AI-industry revenue. The last three panels deliberately divide by $p_XX$,
not by $Y$:
\begin{equation}
 \frac{U}{p_XX}+\frac{M}{p_XX}+\frac{\Pi}{p_XX}=1.
 \label{eq:rewrite-revenue-composition}
\end{equation}
The first figure measures the industry's size relative to final output;
the second separates its inference costs, research expenditure, and net
distribution to its owner.

\begin{figure}[H]
 \centering
 \includegraphics[width=\textwidth]{figures_rewrite/equilibrium_technology_revenue.pdf}
 \caption{Technology, accumulation, and the composition of AI revenue.
 The scenarios and time window match Figure~\ref{fig:rewrite-growth-distribution}.
 Consumption and capital per unit of effective labor use logarithmic scales.
 Capability relative to the frontier and the three revenue shares use linear
 scales. The revenue shares sum to one; negative net distributions, if any,
