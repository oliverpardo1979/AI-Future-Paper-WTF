...
so research compute grows with output. Growth in effective labor at rate
$n+\gamma$ therefore supports AI improvement, which further raises output
and research expenditure. The common denominator $1-\eta-\omega_X$ captures
...
first constructs the BGP and then allows nearby initial stocks off that
path. Along the resulting equilibria, growth rates and the interest rate
converge to the displayed values; they need not equal them at every date.
Consumption and the shadow price adjust to the given stocks, rather than
being freely specified initial conditions. The local convergence result
follows from the same parameter restrictions as part (i); it requires no
additional stability assumption. It does not establish existence or
convergence for arbitrary $K_0,B_0$.
The condition $\eta+\omega_X<1$ is necessary for this positive finite-rate
BGP. The remaining restrictions are sufficient for the global optimality
proof, not asserted to be necessary for equilibrium existence. The proof
also covers $\eta>1/2$: a change of state variable used only in the appendix
permits a concavity argument without changing the RSI technology.

With any finite upper bound, the characterized unit-elastic equilibria have
$g_{Y/N},g_w\to\gamma$. Without an upper bound, RSI can
