\section{Without an AI-efficiency frontier}
...
and both transversality conditions. Removing the frontier does not by itself
imply that $B$ grows without bound.
For any positive variable $V$, I write $g_V=\dot V/V$ for its growth rate.

The results differ in scope. Under complementarity, I establish production
bounds and characterize a possible long-run regime, without proving its
existence. At unit elasticity, I construct an equilibrium. Under substitution,
I identify a parameter range in which no finite-valued developer optimum
exists and explain what remains unresolved outside that range.

\subsection{Complementarity: a production bottleneck without a frontier}
\label{subsec:rewrite-uncapped-complements}

When $0<\sigma<1$, effective labor remains necessary even if AI becomes
arbitrarily efficient. Since the CES exponent is negative, omitting the AI
term from the bracket in equation~\eqref{eq:rewrite-ces} gives an upper bound:
\begin{align}
...
\end{align}
The right-hand side does not depend on $B$.
...
Along any feasible path satisfying production, $L=N$, the research law,
and the resource constraint, $K/(AL)$ and $Y/(AL)$ remain bounded.
...
With any finite frontier, the characterized unit-elastic equilibria have
$g_{Y/N},g_w\to\gamma$. Without a frontier, continued AI improvement can
...
vanishing labor share across the finite-frontier regimes does not extend
...
Complementarity limits output growth even without a frontier, unit elasticity
