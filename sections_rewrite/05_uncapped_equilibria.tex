...
The results differ in scope across substitution regimes. Under complementarity, output-per-worker growth is bounded by the rate of labor-augmenting technological progress, even though there is no upper bound on AI efficiency. At unit elasticity, I give sufficient parameter restrictions for an analytically tractable balanced-growth equilibrium. Under substitution, I show that the AI developer problem may not have a solution even with a finite-time horizon, leading to no equilibrium.
...
improvement. Within the stated restrictions, the reinforcement pushes long-run output-per-worker to grow faster than $\gamma$. Real wages share this premium because unit
...
establishes AI-dominated equilibria whose limiting 
interest rate is 
$\mathcal R(\overline B)$, which
without bound as $\overline B\to\infty$. %This contrasts withpropositions \ref{prop:rewrite-capped-complements-existence} and \ref{prop:rewrite-uncapped-complements-bounds}: undercomplementarity, growth is bounded in the long run by the labor-augmenting technological progress.
...
\end{equation}
If $\sigma>1$, this expression decreases with $s_X$ whenever and reaches its
lower limit as $s_X\to1$.%\footnote{Using$e_X=(1-s_X)/\sigma+\alpha s_X$ from equation~\eqref{eq:rewrite-inverse-demand-elasticity},
%\[
...
%\]
%The denominator is positive for $\sigma>1$. The numerator is affine in $s_X$ and negative at both endpoints: $-(\sigma-1)$ at zero and $-[1-\alpha+\alpha(\sigma-1)^2]$ at one.}
...
Singularities, defined as trajectories where variables reach infinite value in finite time, cannot be equilibria and therefore are not studied in this paper. Still, the following proposition shows that under substituibility and no bound on AI efficiency the AI developer may find no bound in how much it wishes to invest to achieve infinite profits, even if it is constrained to a short period of time.
