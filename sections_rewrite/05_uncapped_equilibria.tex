...
$B\to\infty$. If the aggregate consumption growth rate $g_C=\dot C/C$
...
The proof uses both agents' transversality conditions to rule out consumption
that permanently grows more slowly than effective labor, and shows that
inference and research expenditure vanish relative to output.
It also derives convergence of $K/(AL)$ and $C/(AL)$ to positive constants.
The proposition remains conditional on equilibrium existence, $B\to\infty$,
and a finite limit of consumption growth; it does not assume a BGP.
...
BGP. The bounds $\eta\leq1/2$ and $(1-\alpha)\omega_X\leq1/2$ are sufficient for the
