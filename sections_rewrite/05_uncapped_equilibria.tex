...
\end{equation}
For every $A_0,N_0>0$, there exist $K_0,B_0>0$ and a balanced-growth
equilibrium along which
\begin{align}
...
\end{align}
Fix this BGP. If the local stability and state-projection conditions
\eqref{eq:rewrite-uncapped-unit-local-spectrum}
and~\eqref{eq:rewrite-uncapped-unit-local-projection} in
Appendix~\ref{app:rewrite-uncapped-unit} hold, then for every sufficiently
small neighborhood of the BGP in stationary coordinates, there is an open
neighborhood of its initial stocks such that every $(K_0,B_0)$ in that
stock neighborhood admits an equilibrium that stays in the chosen
stationary neighborhood and converges to the BGP. The required $C_0,q_0$
are continuously differentiable functions of $K_0,B_0$. The equilibrium
is locally unique among trajectories that remain in that neighborhood
and converge to this BGP.
...
being freely specified initial conditions. The local conditions hold at
the calibration in Table~\ref{tab:rewrite-parameters}, as verified in
Appendix~\ref{app:rewrite-uncapped-unit}. This does not establish existence
or convergence for arbitrary $K_0,B_0$.
