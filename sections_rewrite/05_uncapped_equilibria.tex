...
The developer's optimization problem yields a stronger conclusion for
$\eta>\alpha$.

\Needspace{8\baselineskip}
\begin{proposition}[Unbounded research incentives]
\label{prop:rewrite-research-scale}
Suppose $\sigma>1$, $\overline B=\infty$, and $0<\alpha<\eta<1$.
For any $B_0>0$, any finite horizon $T>0$, positive continuous paths of
$K,A,L$, and an integrable interest-rate path, the developer can make
discounted net profits over $[0,T]$ arbitrarily large. Consequently,
there is no infinite-horizon equilibrium with a finite-valued developer
optimum.
\end{proposition}
\begin{proof}
See Appendix~\ref{app:rewrite-proofs},
...
The gains from scaling up early research eventually outgrow its cost.
...
objective, not a finite-time singularity. The result does not resolve
equilibrium existence for $\eta\leq\alpha$.
