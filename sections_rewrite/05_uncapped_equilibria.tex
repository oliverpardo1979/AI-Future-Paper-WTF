\section{Equilibrium without an upper bound on AI productivity}
\label{sec:rewrite-uncapped}

I now remove the upper bound on AI productivity and compare the three
...
The results differ in scope across substitution regimes. Under complementarity, output-per-worker growth is bounded by the rate of labor-augmenting technological progress, even though there is no upper bound on AI productivity. At unit elasticity, I give sufficient parameter restrictions for an analytically tractable balanced-growth equilibrium. Under substitution, I show how growth and interest rates increase with the productivity upper bound and derive a return bound that applies at every date.

\subsection{Complementarity}
\label{subsec:rewrite-uncapped-complements}

When $\sigma<1$, effective labor remains necessary even if AI becomes arbitrarily productive. Equation~\eqref{eq:rewrite-ces} gives an upper bound for $Z$ and $Y$ which does not depend on $B$:
\begin{align}
...
equation~\eqref{eq:rewrite-eq-labor}, the uncapped RSI technology~\eqref{eq:rewrite-uncapped-unit-law},
...
\Needspace{7\baselineskip}
\begin{proposition}[Long-run outcomes under complementarity without an AI productivity upper bound]
...
Inference and research compute vanish as shares of output, not necessarily
in levels. The AI industry nevertheless retains a positive revenue share.
...
and~\eqref{eq:rewrite-uncapped-unit-efficiency-growth} quantify the two
feedback loops. In the technological loop, a higher $B$ makes each unit
of research compute more productive through $BM$. In the economic loop,
better AI raises output, allowing more resources to be devoted to research.
Along this BGP, the developer optimally chooses a positive constant $M/Y$,
so research compute grows with output. Despite autonomous research,
$g_B$ remains proportional to the growth rate of effective labor, $n+\gamma$.
Human labor no longer enters the RSI technology, but at unit elasticity it
remains essential to final-good production. Growth in effective labor
expands the resources available for research, which raises $B$ and further
...
improvement. Within the stated restrictions, both raise $g_B$ and the
growth premium above $\gamma$. Real wages share this premium because unit
elasticity keeps labor's income share constant.

\subsection{Substitution: growth and the AI-productivity upper bound}
\label{subsec:rewrite-uncapped}

With $\sigma>1$, Proposition~\ref{prop:rewrite-equilibrium-regimes}
establishes AI-dominated equilibria whose limiting interest rate and
output-per-worker growth rate are $\mathcal R(\overline B)$ and
$\mathcal R(\overline B)-\rho$. For each fixed $\sigma>1$, both increase
without bound as $\overline B\to\infty$. This contrasts with
Proposition~\ref{prop:rewrite-uncapped-complements-bounds}: under
complementarity, the long-run growth bound does not depend on an
AI-productivity upper bound.

The same function also bounds the interest rate at every date, with or
without an productivity upper bound:
\begin{equation}
...
\end{equation}
For fixed $B$, this expression decreases with $s_X$ and reaches its
lower limit as $s_X\to1$.\footnote{Using
...
equation~\eqref{eq:rewrite-uncapped-dated-return-bound}.
Thus $\mathcal R(B)$ is the lowest return compatible with the production
and monopoly conditions at that productivity level, attained in the
zero-labor-share limit.

If $B$ grows without bound along a trajectory, this inequality requires
...
These conditional implications and the comparison across finite upper
bounds do not establish a finite-time singularity or settle equilibrium
existence without an upper bound.
...
Suppose $\sigma>1$, $\overline B=\infty$, $0<\alpha<1$, and $0<\eta<1$.
...
\item If $\eta>\alpha$, the developer can make discounted net profit
arbitrarily large. Consequently, there is no infinite-horizon equilibrium
...
When $\eta<\alpha$, research costs eventually dominate profit gains on a
fixed horizon. When $\eta>\alpha$, expanding early research makes the
gains outgrow the cost.
Each deviating plan uses finite research expenditure and keeps AI productivity
finite on every finite interval. The obstruction is therefore an unbounded
objective, not a finite-time singularity.

The first case motivates the
choice $\eta=0.20<\alpha=0.33$ in Section~\ref{sec:rewrite-quantitative}:
it avoids this unbounded-payoff mechanism even without an productivity cap.
This is a modeling restriction, not an estimate of $\eta$. With a finite
upper bound, $\eta<\alpha$ is not needed to bound the developer's
finite-horizon value. Part (i) does not establish an infinite-horizon
equilibrium without an upper bound. At $\eta=\alpha$, comparing the growth
...
% unbounded AI productivity is compatible with equilibrium at unit elasticity.
...
% $B\to\infty$. Thus, if AI productivity grows without bound, the interest
...
%
% \Needspace{7\baselineskip}
% \begin{proposition}[Investment restrictions without an productivity upper bound]
...
% from disappearing and keeps research positive. AI productivity must then
...
% inequality contradicts finite AI productivity at every finite date. The
