\subsection{Principal calibration: AI prices and common initial stocks}
\label{subsec:rewrite-low-ai-price-calibration}

I adopt this configuration as the principal calibration. The four economies
share the no-AI Ramsey capital stock, $\omega_X=0.10$, $\eta=0.20$, and
initial AI productivity equal to $1\%$ of its upper bound. Research productivity
is calibrated to the unit-elastic price decline and then held fixed across
$\sigma=0.90,1,1.10,1.50$. The preceding exercises remain illustrative
comparisons; their parameters and equilibrium paths are unchanged.
...
\begin{rewriteParameterTable}{Principal calibration: parameters and initial stocks}{tab:rewrite-low-ai-price-parameters}
$\omega_X$ & 0.10 & AI production weight & Assumed reference value, not fitted to the industry's size.\\
...
$\overline B$ & 1,360.992139 & AI-productivity upper bound & $1.10\mathcal B(1.50)$ evaluated at $\omega_X=0.10$; arbitrary 10\% margin chosen to keep $\sigma=1.50$ AI-dominated.\\
\midrule
...
$B_0$ & 13.609921 & Initial AI productivity & $0.01\overline B$; arbitrary initial distance from the higher bound, not an estimate.\\
$K_0/Y_0$ & 4.728; 3.867; 3.672; 3.466 & Initial capital--output ratio & Calculated from the saved paths; the no-AI reference is 3.30. Not independently targeted.\\
\end{rewriteParameterTable}

The principal calibration uses $\omega_X=0.10$ and
$\omega_L=0.90$, starting AI productivity at $1\%$ of its upper bound.
...
The Ramsey benchmark disciplines the initial capital stock; it does not
require the positive-AI economy to begin on a BGP. I hold $K_0$, $A_0$, $N_0$,
and $B_0$ fixed across the four elasticities, rather than choosing a different
capital stock for each technology. Initial output and the capital--output
ratio can therefore differ across cases. This is a comparison of positive-AI
economies from common stocks, not an adoption experiment that preserves the
old no-AI production technology as an available alternative.
The initial AI-productivity ratio remains illustrative. I retain $\eta=0.20$
...
for subsequent productivity gains relative to the assumed upper bound.
Matching the same proportional price decline does not make the two initial
productivity choices equivalent: $B_0$ affects the initial price and production
allocation, while $\chi$ affects the subsequent improvement in productivity.
...
productivity levels, and upper bounds. Both inherit the same limitations of
...
\begin{figure}[H]
 ...
 \caption{Principal calibration: growth of output, AI production
...
\begin{figure}[H]
 ...
 \caption{Principal calibration: real-wage growth, the net
...
$9.66\%$ after 27 months. Rapid productivity improvement and the distributional
...
\begin{figure}[H]
 ...
 \caption{Principal calibration: labor income, AI net profit,
...
Relative to Section~\ref{subsec:rewrite-price-calibration}, the AI weight,
the initial productivity ratio, the upper bound, and the fitted $\chi$ all
change. That comparison does not isolate the effect of any one parameter;
within the principal calibration, only $\sigma$ varies. Initial AI revenue ranges from $5.2\%$ to
