...
\newif\ifshowlegacysimulations
\showlegacysimulationsfalse
...
\newif\ifshowfullpricebenchmark
\showfullpricebenchmarkfalse
% Restore the earlier price/research-target comparisons with true.
% Their source, data and figures are preserved unchanged.
\newif\ifshowcalibratedrsiscenarios
\showcalibratedrsiscenariosfalse
...
I characterize two long-run equilibrium regimes under a finite upper bound
on AI efficiency. When labor remains a production bottleneck, improvements
in AI efficiency can temporarily raise growth, but the growth rates of output
per worker and real wages eventually return to the exogenously given rate
of labor-augmenting technological progress. When AI and labor are substitutes
and attainable AI efficiency is sufficiently high, wages grow permanently
faster. However, wages grow more slowly than output per worker, and labor's
income share converges to zero. Numerical simulations show that output-per-worker growth may
remain moderate for a long time before accelerating in the AI-dominated
regime or returning to the rate of labor-augmenting technological progress
...
\input{sections_rewrite/08_rsi_activation}
\else
\input{sections_rewrite/08_rsi_design}
\fi
\ifshowcalibratedrsiscenarios
\input{sections_rewrite/09_rsi_half_decline}
\input{sections_rewrite/10_rsi_research_share}
\else
