...
I develop a general-equilibrium growth model to study that feedback. Raw
...
I therefore write production services as the product of capability and inference
...
of the capability--compute architecture in \citet{davidsonetal2026}. It is also informed by
the richer distinction between inference output, training activity, algorithms,
and model capital in \citet{korinekmckelvey2026}. My reduction is substantive:
one stock \(A\) combines model quality and algorithmic efficiency and augments
both uses of compute. I use the term research compute for their broad
...
governing intertemporal idea difficulty are distinct. In my specification,
\(\eta\) is the former;
...
positive income share. At unit elasticity, I derive the growth rates and resource
shares of balanced-growth candidates. An
...
I go beyond this nonexistence result and characterize a conditional AI-dominated
limiting branch of the necessary
...
The timing of this transition is itself a central numerical result. In the
reported finite-boundary approximation, output-per-capita growth remains below
0.5 percent from approximately \(t=10\) to \(t=1330\), and below 2 percent until
approximately \(t=1521\), more than 90 percent of the way to the tail-corrected
boundary estimate near \(t=1669\). It exceeds 10 percent only about 50 model years
before that estimated boundary. The long low-growth phase is not a no-change
phase: by \(t=1000\), when output-per-capita growth is only about 0.15 percent,
aggregate labor income has already fallen from 63.2 to 52.1 percent of output.
The final collapse is nevertheless sharply concentrated: labor's share falls
below 5 percent only about 55 model years before the estimated boundary. The
delayed timing parallels the weak-link transition in \citet{jonestonetti2026},
although their task-automation mechanism is different. These durations are
benchmark model time, not calendar forecasts.

The qualitative connection between automation and labor's income share is
already established. Task automation generates a displacement effect, while
new human tasks can reinstate labor demand \citep{acemoglurestrepo2019}.
Transformative-AI models also show that sufficiently extensive automation can
sharply reduce labor's share \citep{trammellkorinek2023}. Most directly,
\citet{restrepo2025agi} shows that, once compute can perform all bottleneck work,
the expansion of compute can drive growth while labor's income share converges
to zero. I do not claim this qualitative limiting possibility as new.

Across the three regimes, the outer Cobb--Douglas technology fixes the
final-good firm's gross capital payment at \(\alpha Y\). The
final-production elasticity changes how the remaining \((1-\alpha)Y\) is
divided between production labor and gross purchases of AI services. Those
purchases are not developer profit. They finance inference compute and research
inputs before the residual profit is distributed. Combining the final-good
firm's zero-profit condition with the developer's profit identity gives
\[
 Y=(r+\delta)K+wL+wH+\Pi+U+M.
\]
My distributional contribution is to derive how this decomposition
changes across the three substitution regimes and along their numerical paths.
In particular, a fall in \(wL/Y\) is not mechanically a rise in conventional
capital income or in distributed monopoly profit. Aggregate labor income also
includes research wages, while \(U\) and \(M\) are real resource uses rather
than income received by a household in the model. Identifying who ultimately
receives the payments embodied in compute would require an upstream compute
sector and an ownership structure that the baseline does not contain.

The complementarity and unit-elasticity computations solve the same dated
equilibrium system under the capability law specified in the model. The two reported
...
from economic feedback in a multitechnology innovation network. I embed a
one-capability version of that architecture in a decentralized Ramsey economy
with an integrated monopolist. I use this framework to determine inference and
...
limit, I also derive an explicit threshold separating rising from falling
...
At a broader quantitative level, \citet{erdiletal2025gate} integrate a
compute-based model of AI development, AI automation, semi-endogenous growth,
endogenous investment, and adjustment costs. I therefore do not claim to be the
first to combine AI development, automation, investment, and aggregate growth.
My narrower objective is to characterize the decentralized prices, research
choices, feedback conditions, and payments-and-resource-use accounting generated
by the two CES substitution margins and an integrated developer.
