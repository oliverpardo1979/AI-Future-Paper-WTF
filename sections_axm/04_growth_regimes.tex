...
I answer this question with a controlled experiment. I fix \(\omega_X=0.20\)
and start all three economies from the predetermined stocks \(K_0=K^*\) and
\(B_0=B^*\) on the analytical unit-elastic balanced-growth path, with
\(A_0=N_0=1\). At date zero, I set \(\sigma_{XL}\) permanently to \(0.99\),
\(1\), or \(1.01\). The states are therefore identical across cases, whereas
the boundary-value problem selects the jump variables \(C_0\) and \(q_0\).
I do not rescale initial output or any other flow variable after the
perturbation.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures_axm/axm_near_unit_equilibrium_paths.png}
\caption{Permanent near-unit elasticity perturbations from common
unit-elastic balanced-growth stocks. In every case \(\omega_X=0.20\). The
numerical terminal is year 3,000, but I show only years 0--2,500. The output
and wage panels report 100 times the log difference from the
\(\sigma_{XL}=1\) path; the other panels report annual percentages. Appendix
\ref{app:axm-near-unit-equilibrium-paths} describes the equation, horizon, and
refinement audits.}
\label{fig:axm-near-unit-equilibrium}
\end{figure}

The initial distributional response may look surprising because the common
starting stocks imply \(X/(AL)<1\). From
\eqref{eq:axm-near-unit-share-expansion}, a value of \(\sigma_{XL}>1\)
therefore initially lowers \(s_X\) and raises labor's income share; the signs
are reversed for \(\sigma_{XL}<1\). Along the unit-elastic path, however,
\(X/(AL)\) grows and crosses one after roughly 1,225 model years. The
distributional effects then reverse. By year 2,500, 100 times the log gaps in
output per person are \(-7.68\) and \(8.15\) for \(\sigma_{XL}=0.99\) and
\(1.01\), respectively, while labor receives 54.16 and 52.97 percent of
output, compared with 53.60 percent at unit elasticity. Growth and the interest
rate remain close to their unit-elastic values over this window even though
level and distributional gaps have begun to widen.

This experiment establishes continuity on a fixed finite window, not similarity
of the long-run paths. For any fixed date, the three solutions approach one
another as \(\sigma_{XL}\to1\). For a fixed nonunit elasticity, however,
...
finite-window continuation, characterize the conditional tails.
