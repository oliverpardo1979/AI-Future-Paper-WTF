...
ratios. The reported paths end at 1,200 years for \(\sigma_{HM}=1\) and 1,600
...
the largest independent static error is \(1.5\times10^{-9}\), the largest
differential-equation residual is \(1.7\times10^{-5}\), and the range of either
initial jump is below \(1.6\times10^{-7}\) log points. Appendix Table
...
solved from a cold start at terminal dates 1,600, 1,800, and 2,000. The 2,000-year
...
to near one. Human research first rises as a fraction of population and then
...
\(s_M=0.99831\) at the terminal date and \(H/N\) is approximately
\(7.0\times10^{-9}\). Nevertheless, the aggregate labor-income share approaches
0.4473 rather than zero. The Case I computations therefore separate two
...
\midrule
\(\sigma_{HM}=1\) & 0.4187 [0.4178] & 0.000096 & 4.0017 & 0.33260 & 0.35000 & \(6.44\!\times\!10^{-6}\) & 0.44716\\
\(\sigma_{HM}=2\) & 0.5399 [0.5400] & 0.000089 & 4.0007 & 0.33235 & 0.99831 & \(7.04\!\times\!10^{-9}\) & 0.44733\\
\bottomrule
\end{tabular}
...
unit-elasticity benchmarks. Research substitution nearly triples the total gain
and raises the feedback multiplier from 1.245 to 2.286, but even
...
\midrule
\(\sigma_{HM}=1\) & 0.35 & 0.1575 & 0.0394 & 0.1969 & 0.8031 & 1.245\\
\(\sigma_{HM}>1,\ s_M\to1\) & 1.00 & 0.4500 & 0.1125 & 0.5625 & 0.4375 & 2.286\\
\bottomrule
\end{tabular}
...
\(\sigma_{HM}=1\) path ends at 1,200 years and the \(\sigma_{HM}=2\) path at
1,600 years. Proposition
...
0.67 percent, output-per-capita growth approaches 0.17 percent, and \(H/N\)
approaches a positive value. With \(\sigma_{HM}=2\), \(s_M\) approaches one,
capability growth approaches 1.23 percent, output-per-capita growth approaches
0.31 percent, and \(H/N\) approaches zero. The hump in \(H/N\) is a transitional
...
With Cobb--Douglas research the plug-in gain is 0.196875. With substitutable
research it rises from 0.0621 to 0.5625 and remains below one. This diagnostic is
...
\end{equation}
Because \(\det\Omega_\infty=-\eta\theta<0\), this asymptotic exponent matrix has
one positive eigenvalue. This is the network-feedback counterpart of the direct
law \(\dot z\sim\theta h z^2\). Equation \eqref{eq:axm-z-law}, rather than an
imported spectral-radius theorem, remains the proof of finite-time divergence on
the postulated branch.
