...
smooth experiment in \(\sigma_{XL}\). Figure~\ref{fig:axm-noncommuting-limits}
isolates the source of this distinction without imposing another equilibrium
path. I hold \(L\) fixed and vary \(X/L\) in the final-production CES for
elasticities close to one. At every finite ratio, the curves change continuously
with \(\sigma_{XL}\). As \(X/L\) becomes large, however, \(s_X\) moves toward
zero below one, remains at \(\omega_X\) at one, and moves toward one above one.
The payment to production labor, \((1-\alpha)(1-s_X)Y\), and the gross payment
for AI production services, \((1-\alpha)s_XY\), can therefore remain similar
over a range of AI use before separating sharply.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures_axm/axm_noncommuting_limits.png}
\caption{Finite-input continuity and non-commuting limits near unit elasticity.
The figure evaluates the final-production CES exactly at \(\alpha=0.33\),
\(\omega_X=0.20\), and five values of \(\sigma_{XL}\). The horizontal axis is
the log AI-service--labor ratio, not calendar time. This is a technology
experiment, not an equilibrium path: the full equilibrium system determines
how fast \(X/L\) changes. At every finite ratio the CES is continuous in
\(\sigma_{XL}\), but its limiting payment shares differ because the large-
\(X/L\) and \(\sigma_{XL}\to1\) limits do not commute.}
\label{fig:axm-noncommuting-limits}
\end{figure}
