...
\begin{tabular}{P{0.10\textwidth}P{0.11\textwidth}P{0.11\textwidth}P{0.235\textwidth}P{0.335\textwidth}}
\toprule
Parameter & No AI: \(\omega_X=0\) & AI: \(\omega_X=0.20\) & Interpretation & Source\\
\midrule
\(\alpha\) & \multicolumn{2}{c}{0.33} & Elasticity of final output with respect to physical capital & Standard macroeconomic calibration: one-third capital share\\
\(n\) & \multicolumn{2}{c}{0.003} & Exogenous annual population-growth rate & Constant-rate equivalent of the UN world population projection for 2024--2100 \citep{unwpp2024}\\
\(\delta\) & \multicolumn{2}{c}{0.05} & Annual depreciation rate of physical capital & Standard annual calibration, consistent with PWT 11.0 \citep{pwt110}\\
\(\rho\) & \multicolumn{2}{c}{0.04} & Household discount rate & Standard annual preference calibration\\
\(\gamma_A\) & \multicolumn{2}{c}{0.01} & Exogenous annual growth rate of labor productivity & Long-run global technical-progress assumption \citep{oecdlongrun2025}\\
\(\omega_L\) & 1 & 0.80 & CES weight on effective production labor, \(AL\) & CES normalization: \(\omega_L=1-\omega_X\)\\
\(\omega_X\) & 0 & 0.20 & CES weight on AI production services, \(X\) & Scenario calibration; not an empirical estimate \citep{oecdaiinvestment2025,oecdproductivity2026}\\
\(\sigma_{XL}\) & Not applicable & 1 in the benchmark & Elasticity of substitution between \(AL\) and \(X\) & Varied below and above one in the additional AI scenarios\\
\(\eta\) & Not applicable & 0.20 & Elasticity of \(\dot B\) with respect to effective AI research & Model calibration satisfying \(\eta<\alpha\); not estimated\\
\(\chi\) & Not applicable & 0.01 & Productivity scale in the capability law & Time normalization; not estimated\\
\((A_0,N_0)\) & \multicolumn{2}{c}{(1, 1)} & Initial labor productivity and population & Normalizations common to all scenarios\\
\(B_0\) & Not applicable & 1 & Initial AI capability & Normalization of initial AI capability\\
\(K_0\) & Scenario-specific & Scenario-specific & Initial physical-capital stock & Implied by the initial equilibrium after fixing each scenario\\
\bottomrule
\end{tabular}
\end{table}
...
\(\sigma_{XL}=1\). All AI regime exercises retain \(\omega_X=0.20\), hold the
common parameters fixed, and vary \(\sigma_{XL}\). The human-research extension separately uses
\((\omega_H,\omega_M)=(0.65,0.35)\) and reports
\(\sigma_{HM}\in\{1,2\}\).

The value \(\eta=0.20\) is illustrative and lies strictly inside the maintained
region \(\eta<\alpha=0.33\). The normalizations set \(A_0=B_0=N_0=1\) whenever
the corresponding state is present. The initial capital stock is not a common
primitive: \(K_0\) must be chosen separately to place each scenario on its
intended initial equilibrium. The level of \(\chi\) and the initial conditions
affect model time, so the horizontal axes are not calendar forecasts.
