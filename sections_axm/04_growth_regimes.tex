...
sufficiency restrictions in Proposition \ref{prop:axm-equilibrium-sufficiency},
...
The free-boundary algorithm for \((\sigma_{XL},\sigma_{HM})=(1.5,2)\) solves
the dated necessary conditions up to a finite artificial boundary and converges
toward the conditional ratios above. It does not construct an admissible path
for all \(t\geq0\), verify the two infinite-horizon transversality conditions,
or establish global optimality. Under the equilibrium definition in Section
\ref{sec:axm-model}, a branch ending at a finite-time singularity is not an
infinite-horizon equilibrium. I therefore report no simulated trajectory for
\(\sigma_{XL}>1\). The free-boundary code is retained as a diagnostic for the
conditional analytical result, not as a generator of paper figures.
...
\(\sigma_{XL}>1\) equilibrium is constructed.
