...
stationary continuation in the automated-research benchmark.

\begin{proposition}[No regular bounded-capability continuation]
...
This proposition rules out the regular bounded-capability continuation relevant
to the near-unit numerical experiment. The next result characterizes the
unbounded AI-dominated alternative.
...
\begin{enumerate}
 \item capability is bounded and the normalized variables satisfy the limits in
 Proposition \ref{prop:axm-no-bounded-capability-tail}; or
...
does not exclude an asymptotically irregular path, a path without convergent
resource shares, or another terminal regime not used by the algorithm.
