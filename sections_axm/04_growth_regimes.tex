...
Suppose \(\sigma_{XL}>1\), \(G>0\), \(\rho>n\), \(2\eta\leq1\), and
...
\begin{equation}
 ...
 \begin{pmatrix}
  -G\mu&-G\mu\\
  G\mu&\overline r+G\mu
 \end{pmatrix},
 \label{eq:axm-capped-labor-jacobian-blocks}
\end{equation}
where \(r_k<0\), and the remaining root is \(-G\). Each displayed block has
...
\(\varphi(G-\overline g)<0\), the consumption--capital root is
\(\overline c>0\), the root associated with \(\tau\) is \(-G\), and the
...
near a terminal regime. It does not yet establish that the stable manifold can
be continued backward to the paper's original initial stocks. That is a global
continuation problem, not a local-existence condition.
