...
\end{equation}
Keeping this expression as the definition, an equivalent form is
\begin{equation}
 \overline B_c=
 \frac{\mathcal R^{1/\theta}}
 {(1-\alpha)^2
  \omega_X^{\sigma_{XL}/(\sigma_{XL}-1)}}.
 \label{eq:axm-critical-capability-frontier-equivalent}
\end{equation}
Because \(0<\omega_X<1\), the critical frontier diverges as
\(\sigma_{XL}\downarrow1\). The divergence is extremely fast: the exponent on
\(\omega_X\) is of order \(1/(\sigma_{XL}-1)\). Gross substitution can
therefore imply an AI-dominated terminal regime and still require an enormous
capability range before that regime becomes relevant when the elasticity is
only slightly above one.

\begin{table}[H]
 \centering
 \caption{Critical capability frontier above unit elasticity}
 \label{tab:axm-critical-capability-frontier}
 \begin{tabular}{@{}lr@{}}
  \toprule
  \(\sigma_{XL}\) & \(\overline B_c\) \\
  \midrule
  1.01 & \(4.88\times10^{70}\) \\
  1.05 & \(5.90\times10^{14}\) \\
  1.10 & \(6.04\times10^{7}\) \\
 ...
 \begin{minipage}{0.84\textwidth}
  \footnotesize
  \emph{Note:} Author's calculations from
  \eqref{eq:axm-critical-capability-frontier} using the benchmark parameters
  in Table \ref{tab:axm-parameters}. The levels inherit the paper's existing
  normalization of (B); they are analytical thresholds, not empirical
  estimates of AI capability.
 \end{minipage}
\end{table}
...
The terminal restrictions can close an equilibrium BVP only if each terminal
regime has the right saddle-path dimension. The normalization must differ
across the two regimes because their growing scales differ. Let
\(\mu=\eta/(1-\eta)\), and
\(\varphi=(\sigma_{XL}-1)/\sigma_{XL}>0\). Below the threshold, I use
\begin{equation}
 k=\frac{K}{AN},\qquad c=\frac{C}{AN},\qquad
 d=(\overline B-B)AN,\qquad
 \widetilde q=\frac{q}{AN},\qquad
 \tau=(AN)^{-1}.
 \label{eq:axm-capped-labor-normalization}
\end{equation}
Above the threshold, I use
\begin{equation}
 h=\left(\frac{AN}{K}\right)^\varphi,\qquad
 d=(\overline B-B)K,\qquad c=\frac{C}{K},\qquad
 v=\frac{q}{K},\qquad \tau=(AN)^{-1}.
 \label{eq:axm-capped-ai-normalization}
\end{equation}
These are changes of coordinates for the terminal analysis. They do not alter
the definition, timing, or economic meaning of any model variable.

\begin{proposition}[Local finite-cap equilibrium paths]
\label{prop:axm-capped-local-equilibrium}
Suppose \(\sigma_{XL}>1\), \(n+\gamma_A>0\), \(\rho>n\),
\(2\eta\leq1\), and
\(\overline B\neq\overline B_c\). Maintain the positive-consumption and
regularity conditions in Propositions
\ref{prop:axm-capped-terminal-regimes} and
\ref{prop:axm-capped-frontier-research}. Suppose also that the terminal roots
described below are distinct.

If \(\overline B<\overline B_c\), the Jacobian in
\eqref{eq:axm-capped-labor-normalization} is block triangular. Its Ramsey block
and research-gap block are
\begin{equation}
 J_R^-=
 \begin{pmatrix}
  a_k&-\overline c/\overline k\\
  r_k&0
 \end{pmatrix},
 \qquad
 J_B^-=
 \begin{pmatrix}
  -(n+\gamma_A)\mu&-(n+\gamma_A)\mu\\
  (n+\gamma_A)\mu&\overline r+(n+\gamma_A)\mu
 \end{pmatrix},
 \label{eq:axm-capped-labor-jacobian-blocks}
\end{equation}
where \(r_k<0\), and the remaining root is \(-(n+\gamma_A)\). Each displayed block has
negative determinant. The terminal point therefore has three stable and two
unstable roots.

If \(\overline B>\overline B_c\), the root associated with \(h\) is
\(\varphi(n+\gamma_A-\overline g)<0\), the consumption--capital root is
\(\overline c>0\), the root associated with \(\tau\) is
\(-(n+\gamma_A)\), and the
research-gap block is
\begin{equation}
 J_B^+=
 \begin{pmatrix}
 ...
 \end{pmatrix}.
 \label{eq:axm-capped-ai-jacobian-block}
\end{equation}
This block also has negative determinant, so the terminal point again has
three stable and two unstable roots.

In either regime, the stable manifold projects nonsingularly onto the three
predetermined normalized variables. Hence, for predetermined values
sufficiently close to the terminal point, there is a unique local choice of
the two jump variables that generates a regular interior path converging to
the relevant terminal regime. The path satisfies both transversality
conditions. Because the finite-cap developer sufficiency condition holds
strictly in a sufficiently small neighborhood of the frontier, this path is a
local finite-cap equilibrium, not merely a solution of the dated first-order
conditions.
\end{proposition}

The proposition establishes equilibrium paths for initial normalized states
near a terminal regime. It does not by itself establish that the stable
manifold reaches the paper's original initial stocks. I therefore continue the
manifold backward to the common unit-elastic stocks
\(K_0=2.027734\) and \(B_0=0.443671\), with \(A_0=N_0=1\), and apply the full
equilibrium-admission rule. Table
\ref{tab:axm-capped-global-continuation} reports two points from this exercise
at \(\sigma_{XL}=1.10\).
...
\toprule
Case & \(\overline B\) & \(\overline B/\overline B_c\) &
Max. residual & Min. margin & Status\\
\midrule
Moderate & \(60.413\) & \(10^{-6}\) &
\(5.9\times10^{-15}\) & \(0.7202\) & Admitted equilibrium\\
Large subcritical & \(3.021\times10^7\) & \(0.5\) &
\(2.8\times10^{-14}\) & \(-0.00631\) & Rejected candidate\\
\bottomrule
\end{tabular}
\caption{Global finite-cap continuation from the common initial stocks. The
moderate-frontier margin is the minimum from the counterfactual developer
audit over the reachable capability interval. For the large frontier, the
reported negative margin occurs on the candidate path itself, so a broader
counterfactual search is unnecessary. Numerical details are in Appendix
\ref{app:axm-finite-cap-algorithm}.}
\label{tab:axm-capped-global-continuation}
\end{table}

The moderate-frontier calculation connects the initial stocks to the
labor-supported terminal regime. Extending its terminal horizon by 500 years
changes the two initial jump variables by at most \(1.3\times10^{-8}\) and the
four solver coordinates over years 0--2,500 by at most
\(2.9\times10^{-6}\). Both transversality objects have terminal log values
below \(-215\) and then decay at rate \(n-\rho=-0.037\). The finite-cap
developer condition also holds with a large positive margin. I therefore admit
this numerical path as a finite-cap equilibrium.

The large-frontier calculation is equally accurate as a solution of the
canonical equations, and its initial jumps are stable when the horizon is
extended. It nevertheless fails the developer condition on the path itself:
the minimum occurs when \(B/\overline B\) is about \(0.137\) and \(s_X\) is
about \(0.739\). The path is therefore not admitted as an equilibrium. This
distinction is substantive. A finite frontier guarantees local sufficiency
near its endpoint, but it need not preserve that sufficiency along the entire
transition from a distant initial capability.
...
The calculation establishes that the finite-cap construction can produce an
equilibrium from the paper's initial stocks for at least one fixed frontier. It
does not yet construct the sequence of equilibria with
\(\overline B\to\infty\) required by the next proposition. The current
sufficiency argument stops before that limit because the concavity gate fails
for the large-frontier candidate. I therefore do not use either calculation as
a gross-substitutes transition figure and do not infer existence or
nonexistence of an equilibrium above \(\overline B_c\).
...
\begin{proof}
Let \(\mathcal L=AN\). Because \(n+\gamma_A>0\) and capability is
