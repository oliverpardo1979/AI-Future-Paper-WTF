...
percent over the same interval.
...
and logarithmic vertical scales. The interest-rate panel uses a focused linear
scale. Model time is not a calendar forecast.}
...
The distributional effect is much larger and occurs immediately: labor loses
13.4 percentage points of output, while gross physical-capital income remains
at 33 percent. AI profit and the resources used for inference and research
receive the difference. Thus this experiment generates a large change in the
distribution of output long before it generates a large change in output.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures_axm/axm_ai_adoption_mechanism_distribution.png}
\caption{Growth, capability, and the distribution of output after AI adoption.
Growth and \(qB/Y\) use focused linear scales; \(B/B_0\) uses a logarithmic
scale. The final panel decomposes output into gross physical-capital income,
labor income, distributed AI profit, inference compute, and research compute.
The first two bars report the no-AI allocation and the allocation immediately
after adoption. The third reports the analytical AI balanced-growth allocation;
it is not the allocation at year 250.}
