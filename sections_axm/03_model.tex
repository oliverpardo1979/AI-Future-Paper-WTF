...
the predetermined stocks \(K_0,B_0>0\). The exogenous paths are
\(A_t=A_0e^{\gamma_At}\) and \(N_t=N_0e^{nt}\). A regular interior benchmark
...
satisfies the following system, with the differential equations understood
almost everywhere:
\begin{subequations}
...
\begin{align}
 ...
 p_X(1-e_X)&=\frac{1}{B},& X&=BU,\displaybreak[4]\\
 q\chi\eta B(BM)^{\eta-1}&=1,
...
The block contains no separate resource identity because that identity is
exactly the law of capital accumulation. Final-good zero profit, the household budget,
and the consolidated income identity follow from constant returns, the factor-
price equations, market clearing, and the definition of \(\Pi\); they are not
additional equilibrium restrictions. Given \((A,N,K,B,C,q)\), the twelve
...
Fix the model's parameters, the exogenous initial values \(A_0,N_0>0\), and
the predetermined stocks \(K_0,B_0>0\). The exogenous paths are
\(A_t=A_0e^{\gamma_At}\) and \(N_t=N_0e^{nt}\). A regular interior equilibrium
with human research is an admissible infinite-horizon trajectory
\begin{equation}
