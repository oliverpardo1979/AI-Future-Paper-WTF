...
\subsubsection{Benchmark equilibrium in one block}

I now collect the complete benchmark system. This repeats conditions derived
above so that the reader can see in one place what determines an equilibrium
trajectory. Given positive predetermined initial conditions, the dated
conditions are
\begin{subequations}
\label{eq:axm-canonical-system-benchmark}
\begin{align}
 \dot N&=nN,\qquad \dot A=\gamma_AA,
 \label{eq:axm-equilibrium-exogenous-block}\\
 L&=N,\\
 Z&=\left[\omega_L(AL)^{(\sigma_{XL}-1)/\sigma_{XL}}
 +\omega_XX^{(\sigma_{XL}-1)/\sigma_{XL}}\right]^{
 \sigma_{XL}/(\sigma_{XL}-1)},\\
 Y&=K^\alpha Z^{1-\alpha},
 \label{eq:axm-equilibrium-production-block}\\
 s_X&=\frac{\omega_XX^{(\sigma_{XL}-1)/\sigma_{XL}}}
 {\omega_L(AL)^{(\sigma_{XL}-1)/\sigma_{XL}}
 +\omega_XX^{(\sigma_{XL}-1)/\sigma_{XL}}},\\
 e_X&=\frac{1-s_X}{\sigma_{XL}}+\alpha s_X,
 \label{eq:axm-equilibrium-share-block}\\
 r&=\alpha\frac{Y}{K}-\delta,\\
 w&=(1-\alpha)(1-s_X)\frac{Y}{L},\qquad
 p_X=(1-\alpha)s_X\frac{Y}{X},
 \label{eq:axm-equilibrium-price-block}\\
 p_X(1-e_X)&=\frac{1}{B},\\
 X&=BU,\\
 q\chi\eta B(BM)^{\eta-1}&=1,
 \label{eq:axm-equilibrium-developer-static-block}\\
 \dot B&=\chi(BM)^\eta,\\
 \dot K&=Y-C-U-M-\delta K,
 \label{eq:axm-equilibrium-state-block}\\
 \frac{\dot C}{C}&=n+r-\rho,\\
 \frac{\dot q}{q}&=r-\frac{X}{qB^2}
 -\eta\frac{\dot B}{B}
 =r-\frac{U}{qB}-\eta\frac{\dot B}{B},
 \label{eq:axm-equilibrium-jump-block}\\
 \Pi&=p_XX-U-M,\\
 Y&=C+\dot K+\delta K+U+M.
 \label{eq:axm-equilibrium-accounting-block}
\end{align}
\end{subequations}
At \(\sigma_{XL}=1\), the CES equations in
\eqref{eq:axm-equilibrium-production-block}--
\eqref{eq:axm-equilibrium-share-block} are understood at their continuous limit:
\(Z=(AL)^{\omega_L}X^{\omega_X}\) and \(s_X=\omega_X\).

The predetermined variables satisfy
\begin{equation}
 K(0)=K_0,\qquad A(0)=A_0,\qquad
 B(0)=B_0,\qquad N(0)=N_0,\qquad K_0,A_0,B_0,N_0>0.
 \label{eq:axm-benchmark-initial-conditions}
\end{equation}
The initial values \(C(0)\) and \(q(0)\) are not additional primitives. They
must be selected so that the household and developer satisfy their terminal
conditions:
\begin{equation}
...
Fix the model's parameters and positive initial stocks
\((K_0,A_0,B_0,N_0)\). A regular interior benchmark equilibrium is an
infinite-horizon trajectory
\begin{equation}
 \left\{K_t,A_t,B_t,N_t,C_t,q_t,L_t,U_t,M_t,X_t,Z_t,Y_t,
 r_t,w_t,p_{X,t},s_{X,t},e_{X,t},\Pi_t\right\}_{t\geq0}
 \label{eq:axm-benchmark-equilibrium-trajectory}
\end{equation}
with the following properties:
\begin{enumerate}[label=(\roman*),leftmargin=*,itemsep=0.25\baselineskip,
 topsep=0.4\baselineskip,parsep=0pt]
 \item The trajectory is admissible; the quantities and the prices
 \(w,p_X,q\) are strictly positive; \(s_X,e_X\in(0,1)\);
 \(K,A,B,N,C,q\) are absolutely continuous; and the remaining dated variables
 have the regularity required by the displayed equations. The net interest
 rate \(r\) and developer payout \(\Pi\) may have either sign.
 \item The predetermined variables satisfy
 \eqref{eq:axm-benchmark-initial-conditions}. The initial values \(C(0)\) and
 \(q(0)\) are selected as part of the equilibrium.
 \item The algebraic equalities and differential equations in
 \eqref{eq:axm-canonical-system-benchmark} hold at every date, with
 differential equations understood almost everywhere.
 \item The two transversality conditions in \eqref{eq:axm-tvc} hold.
 \item The household and developer solve their respective optimization
 problems, final-good firms maximize profits, and all markets clear.
\end{enumerate}
Regularity means that the maximum-principle multipliers are normal and the
displayed derivatives exist wherever they are used.
\end{definition}

For general parameter values, the displayed equations and terminal conditions
are necessary but need not be sufficient for global dynamic optimality. I call
an admissible path that satisfies them a candidate equilibrium path. I derive
sufficient conditions for global optimality only after presenting the
human-research extension.
...
Fix the model's parameters and positive initial stocks
\((K_0,A_0,B_0,N_0)\). A regular interior equilibrium with human research is an
infinite-horizon trajectory
\begin{equation}
 \left\{K_t,A_t,B_t,N_t,C_t,q_t,L_t,H_t,U_t,M_t,X_t,E_t,Z_t,Y_t,
...
\begin{enumerate}[label=(\roman*),leftmargin=*,itemsep=0.25\baselineskip,
 topsep=0.4\baselineskip,parsep=0pt]
 \item The trajectory is admissible; \(K,A,B,N,C,q\) are absolutely
 continuous; all quantities and the prices \(w,p_X,P_E,q\) are strictly
 positive; \(s_X,s_M,e_X\in(0,1)\); and the remaining dated variables have the
 regularity required by the displayed equations. The net interest rate \(r\)
 and developer payout \(\Pi\) may have either sign.
 \item The predetermined variables satisfy \(K(0)=K_0\), \(A(0)=A_0\),
 \(B(0)=B_0\), and \(N(0)=N_0\). The initial values \(C(0)\) and \(q(0)\)
 are selected as part of the equilibrium.
 \item At every date the trajectory satisfies labor-market clearing
 \eqref{eq:axm-labor-market}; final production, the AI share, factor prices,
 and inverse-demand elasticity in \eqref{eq:axm-final-production},
 \eqref{eq:axm-sx}, \eqref{eq:axm-factor-prices}, and
 \eqref{eq:axm-inverse-elasticity}, with \(e_X=\varepsilon_X^{-1}\); the
 service identity and monopoly condition in \eqref{eq:axm-ai-service} and
 \eqref{eq:axm-monopoly-foc}; the effective-research technology, research
 price, conditional demands, research share, and scale condition in
 \eqref{eq:axm-effective-research}, \eqref{eq:axm-research-price},
 \eqref{eq:axm-research-demands}, \eqref{eq:axm-sm}, and
 \eqref{eq:axm-research-scale-foc}; and the household budget, developer profit,
 and resource identity in \eqref{eq:axm-household-budget},
 \eqref{eq:axm-developer-profit}, and \eqref{eq:axm-resource}. All CES
 relations are understood at their stated unit-elasticity limits.
 \item The differential equations in \eqref{eq:axm-canonical-system} hold
 almost everywhere, and the two transversality conditions in
 \eqref{eq:axm-tvc} hold.
 \item The household and developer solve their respective optimization
 problems, final-good firms maximize profits, and all markets clear.
\end{enumerate}
Regularity has the same meaning as in the benchmark definition.
\end{definition}

\Needspace{6\baselineskip}
The first four clauses of the definition collect the dated first-order, state,
market, accounting, initial, and terminal conditions. A path satisfying those
clauses is a candidate equilibrium path. The final clause distinguishes an
equilibrium from a candidate by requiring global agent optimality. Proposition
