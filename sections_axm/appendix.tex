...
201-node mesh. Candidate levels are chosen by a small least-squares calculation
...
\(2\times10^{-5}\). The primary horizons are \(T=1{,}200\) for
\(\sigma_{HM}=1\) and \(T=1{,}600\) for \(\sigma_{HM}=2\). They are re-solved at
\(T\in\{1{,}000,1{,}200,1{,}400\}\) and
\(T\in\{1{,}400,1{,}600,1{,}800\}\), respectively. The complementary-input
calculations use \(\sigma_{XL}=0.75\), a solver tolerance of \(10^{-6}\), and
independent cold starts at \(T\in\{1{,}600,1{,}800,2{,}000\}\) for each value of
...
\(T=2{,}000\) paths are displayed. Saved output is evaluated from the collocation
...
paths, the largest stored solver static-equation error is
\(4.8\times10^{-11}\), while the independent row-by-row reconstruction has a
maximum static error of \(1.5\times10^{-9}\). The largest differential-equation
residual is \(1.7\times10^{-5}\), and both imposed terminal-ratio errors are below
\(5.1\times10^{-7}\). Every saved quantity remains positive, and the monopoly
...
Across the two displayed \(T=2{,}000\) paths, the largest reconstructed static
error is \(2.92\times10^{-11}\), the largest independently differentiated
dynamic error is \(5.68\times10^{-7}\), and the largest monopoly first-order
condition error is \(2.50\times10^{-11}\). The largest solver root-mean-square
residual is \(9.43\times10^{-7}\), and the largest horizon-induced initial-jump
range is \(7.7\times10^{-8}\) log points. Every prespecified acceptance gate
...
system.
The package list in \path{requirements-numerical.txt} does not pin software
versions, so the paper does not claim bit-for-bit replication across computing
environments.
...
so, for example, 0.05924 corresponds to 5.924 percent. The final column reports
