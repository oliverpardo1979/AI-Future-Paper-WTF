...
A finite capability frontier makes the last case analytically tractable without
changing the meaning of capability. For each fixed \(\overline B\), the state
approaches but never crosses the frontier. A critical value separates a
labor-supported terminal regime from an AI-dominated \(AK\)-type regime. This
construction is an equilibrium regularization, not a robustness exercise. It
also clarifies the uncapped boundary. A numerical continuation from a moderate
frontier reaches the paper's initial stocks and passes the dated equations,
both transversality conditions, horizon checks, and the developer's global
sufficiency test. A much larger subcritical frontier still produces an accurate
solution of the canonical equations, but the developer's concavity condition
fails along the transition. Thus the finite-cap construction establishes an
equilibrium for a fixed moderate frontier, but not a sequence of equilibria as
\(\overline B\to\infty\). Even if such a sequence exists, capped paths can
converge to the original equations only on fixed pre-singular intervals, while
their terminal growth and interest rates diverge.
